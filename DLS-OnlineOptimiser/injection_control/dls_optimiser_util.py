...
@author: James Rogers and David Obee

This version of util has injection control. The important class to read through is dls_machine_interactor_bulk_base_inj_control. 
This contains the methods for starting and stopping injection. The measure_results function is also an important part to 
understand; this is how the interactor deals with noisy data.
...
def measure_results(measurement_vars, get_command):
    ...
    for i in range(len(measurement_vars)):                               
        ...
        #first set of mean+standard deviation    
...
###################################################  MAIN INTERACTOR FOR INJECTION CONTROL  #############################################
...
class dls_machine_interactor_bulk_base_inj_control:
    ...
    def get_mr(self):
        ...
        while beam_current < beam_current_bounds[0]:
            print 'waiting for beam current to rise above ',beam_current_bounds[0]
            cothread.Sleep(1)
            beam_current = get_command('SR-DI-DCCT-01:SIGNAL')
            print '...'
        ...
        mrs_noinj = measure_results(self.measurement_vars_noinj, abstract_caget)
