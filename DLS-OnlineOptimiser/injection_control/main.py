...
class modified_interactor(util.dls_machine_interactor_bulk_base_inj_control):
    """
    This class is defining the interactor between the algorithm file and the util file.
    """
...
class main_window(Tkinter.Frame):
    ...
    def initUi(self):
        ...
        self.parent.title("DLS Machine Optimiser with Injection Control")
        ...
        self.Toutput_params.heading("inj", text="Injection Setting")
        ...
        #-------------------------------------additional lifetime proxy objective-------------------------------------------#
        self.btn_output_params_add = Tkinter.Button(self.parent, text="Add Lifetime", command=self.show_add_lifetime_window)  
        #-------------------------------------------------------------------------------------------------------------------#                  
        self.btn_output_params_add.grid(row=2, column=3, rowspan=1, sticky=Tkinter.E+Tkinter.W+Tkinter.N+Tkinter.S)
    ...
    #next three functions make appropriate winbdows appear on screen
...
class add_obj_func(Tkinter.Frame):
    ...
    def add_pv_to_list(self):
        ...
        iid = the_main_window.Toutput_params.insert('', 'end', text=self.i0.get(), values=(self.i1.get(), self.i2.get(), mrr.max_min_text, mrr.inj_setting_text))
        mrr.list_iid = iid
    ...
    def show_lifetime_options_window(self):
        """
        Show add_lifetime window if called upon
        """
...
class add_lifetime(Tkinter.Frame):
    """
    ...
    def initUi(self):
        ...
        self.b1 = Tkinter.Button(self.parent, text="Cancel", command=add_lifetime_window.withdraw)
    ...
    def add_pv_to_list(self):
        ...
        map.define_number_of_bunches(float(number_of_bunches))
        util.update_beam_current_bounds(float(beam_current_min), float(beam_current_max))
               
        add_lifetime_window.withdraw()
...
add_lifetime_window = Tkinter.Toplevel(root)
add_lifetime_frame = add_lifetime(add_lifetime_window)
add_lifetime_window.withdraw()
